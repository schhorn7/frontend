      ...
      <div v-if="loans" class="w-full bg-white rounded-lg p-10 space-y-4">
        ...
        <div class="space-y-4">
          <div class="flex justify-start items-center gap-x-5">
            ...
            <div
              v-if="!loans.lender?.profile_picture"
              :class="[
                'flex-shrink-0 flex items-center justify-center rounded-full w-12 h-12 text-white font-bold text-lg',
                getRandomBg(loans.lender?.first_name),
              ]"
            >
              {{ loans.lender?.first_name.charAt(0).toUpperCase()
              }}{{ loans.lender?.last_name.charAt(0).toUpperCase() }}
            </div>
            <!--show profile path-->
            <div
              v-else
            ...
            >
              <img
                :src="fullUrl(loans.lender?.profile_picture)"
                alt="Profile"
                class="object-cover w-full h-full rounded-full"
              />
            </div>
            <span class="text-lg font-medium text-gray-900"
              >{{ loans.lender?.first_name }}
              {{ loans.lender?.last_name }}</span
            >
          </div>
...
onMounted(async () => {
  try {
    const res = await $fetch(
      `${config.public.apiBase}/getLoanAfterApproveforBorrower/${route.params.paybackLoanId}`,
      {
        headers: {
...
const handleFund = async () => {
  try {
    const res = await $fetch(
      `${config.public.apiBase}/payback/${route.params.paybackLoanId}/${id}`,
      {
        method: "POST",
        body: {
